	...
	//blackboard()
{
	...
	brain = new DecisionMakingAlgorithm();
	if (temp == AgentType::PLAYER) { max_velocity = 250.f; mass = 0.035f; }
}
...
void Agent::setPathBehavior(Pathfinding* path)
{
	pathfinding = path;
}
void Agent::setSensorySystem(SensorySystem* _sensory_system)
{
	sensory_system = _sensory_system;
}
void Agent::setBrain(DecisionMakingAlgorithm* _brain)
{
	brain = _brain;
}
...
void Agent::update(float dtime, SDL_Event *event)
{
	...
	{
		// Update SensorySystem
		sensory_system->update(this);
		brain->update(this, dtime);
		//Update Brain(FSM)
	}

	if (haveWeapon) { currentTime += dtime; }
	if (currentTime >= maxTime) { currentTime = 0; haveWeapon = false; }
...
void Agent::setHaveWeapon(bool weapon)
{
	haveWeapon = weapon;
}
